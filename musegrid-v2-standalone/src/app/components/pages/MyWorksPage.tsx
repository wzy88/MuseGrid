...
function buildContributionItems(work: GeneratedWork) {
  ...
  return [
    ...
    { step: '制作', avatar: '人声策展师', lv: 3, w: 25, output: '', edit: '', at: '07/10 17:56', adopt: 91 },
  ];
}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden', background: C.bg0 }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '20px 28px', display: 'flex', flexDirection: 'column', gap: 8 }}>
        {filtered.map((work) => (
          <GlassCard key={work.id} style={{ display: 'flex', alignItems: 'center', gap: 16, padding: '12px 16px', cursor: 'pointer' }} onClick={() => setSel(work)}>
...
function WorkResult({
  ...
  const stageFacts = buildStageFacts(work);
  const stageMetrics = buildStatCards(work);
  const revenueParts = buildRevenueParts(work);
  const contributionItems = buildContributionItems(work);
  ...
  const stagePrompt = work.finalPrompt || '暖色流行女声，保留副歌空旷留白和雨夜里带一点反光的空气感。';
  const stageDescription = getProjectSummary(work);
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 1.45fr) 314px', gap: 18, alignItems: 'start' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '220px minmax(0,1fr)', gap: 16 }}>
              ...
              >
                ...
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 10, marginBottom: 12 }}>
                  {[
                    { label: '完成时间', value: formatTimestamp(work.updatedAt || work.createdAt) },
                    { label: '声音方向', value: `${work.tags[2] || '女声'} / 氛围靠前` },
                    { label: '时长', value: formatDuration(work.duration) },
            ...
            <GlassCard radius={20} style={{ padding: 18, background: 'linear-gradient(180deg, rgba(19,27,39,0.96), rgba(17,24,34,0.94))', border: '1px solid rgba(255,255,255,0.06)' }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 12 }}>
                ...
                <Tag variant="dim">{formatDuration(work.duration)}</Tag>
              </div>
              ...
              <div style={{ borderRadius: 20, background: 'linear-gradient(180deg, rgba(38,47,60,0.92), rgba(21,30,40,0.96))', padding: 18, boxShadow: 'inset 0 1px 0 rgba(255,255,255,0.08)' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 12 }}>
                  <div style={{ width: 44, height: 44, borderRadius: 14, background: `radial-gradient(circle at 30% 30%, ${MINT} 0%, ${work.color} 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 22px ${MINT_DIM}` }}>
                    <Music3 size={20} color="#fff" />
                  </div>
                  <div style={{ minWidth: 0 }}>
                    <p style={{ color: C.t0, fontSize: 18, fontWeight: 600, marginBottom: 4 }}>{work.title}</p>
                    <p style={{ color: C.t3, fontSize: 12 }}>{work.generationSource?.startsWith('minimax') ? '真实音频 Demo' : 'MuseGrid Tester · sample Demo'}</p>
                  </div>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  ...
                  <div style={{ flex: 1 }}>
                    <Waveform bars={54} progress={playing ? 0.42 : 0} height={30} seed={work.seed} activeColor={MINT} inactiveColor="rgba(255,255,255,0.12)" />
                  </div>
                  <span style={{ color: C.t2, fontSize: 12, minWidth: 42, textAlign: 'right' }}>{formatDuration(work.duration)}</span>
                </div>
              </div>
            </GlassCard>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1fr)', gap: 18 }}>
              <GlassCard radius={18} style={{ padding: 18, background: 'linear-gradient(180deg, rgba(18,26,38,0.96), rgba(14,20,29,0.94))' }}>
                ...
                <pre style={{ margin: 0, whiteSpace: 'pre-wrap', color: C.t1, fontSize: 12, lineHeight: 1.8, fontFamily: "'Noto Sans SC',sans-serif" }}>{buildPromptSummary(work)}</pre>
              ...
              <GlassCard radius={18} style={{ padding: 18, background: 'linear-gradient(180deg, rgba(18,26,38,0.96), rgba(14,20,29,0.94))' }}>
                ...
                <pre style={{ margin: 0, whiteSpace: 'pre-wrap', color: C.t1, fontSize: 12, lineHeight: 2, fontFamily: "'Noto Sans SC',sans-serif" }}>
                  {work.lyrics || '歌词尚未确认，完成制作后会显示在这里。'}
                </pre>
              </GlassCard>
            </div>
          </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <GlassCard radius={18} style={{ padding: 16, background: 'linear-gradient(180deg, rgba(20,28,40,0.96), rgba(14,18,26,0.96))' }}>
              ...
              <p style={{ ...T.caption, color: C.t3, marginBottom: 12 }}>数据更新于 {formatTimestamp(work.updatedAt || work.createdAt)}</p>
            ...
            <GlassCard radius={18} style={{ padding: 16, background: 'linear-gradient(180deg, rgba(20,28,40,0.96), rgba(14,18,26,0.96))' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 14, marginBottom: 14 }}>
                <div style={{ width: 112, height: 112, borderRadius: '50%', background: donutGradient, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                  <div style={{ width: 64, height: 64, borderRadius: '50%', background: '#121A24', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
                    <p style={{ color: C.t0, fontSize: 16, fontWeight: 700 }}>{formatNumber(Math.max(work.plays, DAY_DATA[0]))}</p>
                  </div>
                </div>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, minmax(0,1fr))', gap: 10 }}>
                <div style={{ borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', padding: 12, background: 'rgba(255,255,255,0.03)' }}>
                  ...
                  <p style={{ color: C.t0, fontSize: 22, fontWeight: 700 }}>{formatNumber(Math.max(work.plays, DAY_DATA[0]))}</p>
                </div>
                <div style={{ borderRadius: 12, border: '1px solid rgba(255,255,255,0.06)', padding: 12, background: 'rgba(255,255,255,0.03)' }}>
                  <p style={{ ...T.label, color: C.t3, marginBottom: 6, textTransform: 'none' }}>预估二创</p>
                  <p style={{ color: C.t0, fontSize: 22, fontWeight: 700 }}>{formatNumber(Math.max(247, Math.round(work.likes * 0.3)))}</p>
                </div>
              </div>
            </GlassCard>
