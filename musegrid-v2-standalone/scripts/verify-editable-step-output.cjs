...
(async () => {
  ...
  const reviseClass = await reviseButton.getAttribute('data-state');
  assert(reviseClass === 'ready', 'continue revision button should enter an emphasized ready state after direct edits');

  const compareButton = page.getByRole('button', { name: /选择对比分身/ });
  assert(await compareButton.isDisabled(), 'comparison avatar button should be disabled in this version');
