...
(async () => {
  ...
  assert(await cardSummonButtons.count() >= 3, 'avatar cards should expose direct summon buttons');
  ...
  await cardSummonButtons.first().click();
  await page.waitForTimeout(400);
  const body = await page.locator('body').innerText();
  assert(body.includes('当前环节：作词') || body.includes('选择作词方式'), 'card summon should navigate into production flow');
